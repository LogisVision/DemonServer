...
class MQTTHandler:
    ...
    def on_message(self, client, userdata, message):
        ...
        if topic == self.statusA_response_topic:
            ...
            Logger().info(f"Updated Jetson status: {self.jetsonA_status}")
        elif topic == self.statusB_response_topic:
            self.jetsonB_status = message_payload
            Logger().info(f"Updated Jetson status: {self.jetsonB_status}")
            Logger().info(f"Updated Jetson status: {self.jetsonA_status}")
    ...
    def send_command(self, command_message, robot_type):
        ...
        if robot_type == 'A':
            ...
            self.client.publish(self.statusB_request_topic, json_data)
